...
void render_dungeon(dungeon_t *d) {
  ...
  for (i = 0; i < DUNGEON_Y; i++) {
    for (j = 0; j < DUNGEON_X; j++) {
      ...
      /* Objects */
      if (d->map_visible[i][j] || FOV_get_fog() == false) {
        object *object = d->object_map[i][j];

        if (object != NULL) {
          attron(A_BOLD);
          attron(COLOR_PAIR(object->color));
          mvprintw(y, x, "%c", object_symbol[object->type]);
          attron(COLOR_PAIR(0));
          attroff(A_BOLD);
...
int generate_rooms(dungeon_t *d) {
  ...
  for (i = 0; i < d->num_rooms; i++) {
    ...
    do {
      ...
      int j;
      for (j = (i - 1); j >= 0; j--) {
        if ((_intersects = intersects(&(d->rooms[i]), &(d->rooms[j]), 2))) {
          break;
        }
      }
  ...
  for (i = 1; i < d->num_rooms; i++) {
    ...
    for (j = (i - 1); j >= 0; j--) {
      if (j == (i - 1)) {
        ...
        int distance = sqrt(
            pow((d->rooms[i].coordinates.x - connected_rooms[j].coordinates.x),
                2) +
            pow((d->rooms[i].coordinates.y - connected_rooms[j].coordinates.y),
                2));

        int closest_distance = sqrt(
            pow((d->rooms[i].coordinates.x - closest_room.coordinates.x), 2) +
            pow((d->rooms[i].coordinates.y - closest_room.coordinates.y), 2));
    ...
    int y = (i % 2 == 0) ? d->rooms[i].coordinates.y
    ...
    while (x != x_closest) {
      ...
      d->map[y][x] =
          (d->map[y][x] == ter_floor_room) ? ter_floor_room : ter_floor_hall;
    }
...
int generate_hardness(dungeon_t *d) {
  ...
  for (i = 0; i < DUNGEON_Y; i++) {
    for (j = 0; j < DUNGEON_X; j++) {
      ...
      if (t == ter_rock || t == ter_wall_vertical || t == ter_wall_horizontal) {
...
point_t get_valid_point_object(dungeon_t *d) {
  point_t p;
  ...
  for (i = 0; i < DUNGEON_Y; i++) {
    for (j = 0; j < DUNGEON_X; j++) {
      if (!fread(&(d->hardness_map[i][j]), sizeof(uint8_t), 1, f)) {
        fprintf(stderr, "Error: could not load <hardness_map[%d][%d]>. (%d)\n",
                i, j, errno);
        exit(-11);
      }
    }
  ...
  for (i = 0; i < DUNGEON_Y; i++) {
    for (j = 0; j < DUNGEON_X; j++) {
      if (!fwrite(&(d->hardness_map[i][j]), sizeof(uint8_t), 1, f)) {
        fprintf(stderr, "Error: could not save <hardness_map[%d][%d]>. (%d)\n",
                i, j, errno);
        exit(-41);
      }
    }
